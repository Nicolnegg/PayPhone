    ...
    connection.query('SELECT P.nombre, P.precio_venta, C.cantidad FROM carrito C, producto P WHERE C.usuario_id = ? AND P.producto_id = C.producto_id', [usuario_id], async (error, results) => {
        ...
        for (const producto of Object.keys(productostabla)) {
            ...
            if(producto != "total"){
                ...
                <td>${formateador.format(productostabla[producto].precio_venta)}</td>
                <td>${formateador.format((productostabla[producto].precio_venta)*(productostabla[producto].cantidad))}</td>
                </tr>`;
            }
